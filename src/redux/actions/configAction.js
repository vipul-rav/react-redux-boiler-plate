...
export const fetchContent = createAction(actionTypes.FETCH_CONTENT, function prepare(navigate) {
    return {
        meta: {
            loading: true
        },
        payload: {
            ...
            method: GET_METHOD,
            onSuccess: (response)=>contentSuccess(response,navigate),
            onFailure: actionFailed
        }
    };
});

const contentSuccess = createAction(actionTypes.FETCH_CONTENT_SUCCESS, function prepare(response,navigate) {
    return {
        meta: { loading: false },
        payload: {response,navigate}
    };
});
